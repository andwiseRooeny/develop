...
모든 워크플로우는 **YAML Frontmatter 아래**, **실행 단계 전**에 다음 형식으로 참조 문서를 명시해야 합니다:

```markdown
<!-- 📚 참조 문서 시작 -->
**[Header]**: [개발-공통-준수-규칙](./개발-공통-준수-규칙.md)
**[Architecture]**: [분석-아키텍처-가이드](../_ref/common/분석-아키텍처-가이드.md)
<!-- 📚 참조 문서 끝 -->
```

- **태그**: `[Header]`, `[Architecture]`, `[Template]` 등 역할별 태그를 사용합니다.
- **링크**: 상대 경로를 사용하여 참조 문서를 연결합니다.
- **마커**: 반드시 `<!-- 📚 참조 문서 시작 -->`과 `<!-- 📚 참조 문서 끝 -->` 사이에 배치합니다.
