...
이 문서는 프로젝트의 주요 폴더별 역할과 구조를 정의합니다.

## 📁 주요 경로 일람

### 1. 자바 구현 경로 (`client/unist/java/custom`)
- **역할**: 프로젝트의 커스텀 비즈니스 로직(Controller, Service, Mapper 등)이 구현되는 핵심 공간입니다.
- **주요 하위 구조**:
  - `app/`: 어플리케이션 레벨 서비스 구현
    - `campusmap/impl/`: 캠퍼스맵 기능 서비스 (`CampusMapSaveServiceImpl.java`, `CampusMapSelectListServiceImpl.java`)
  - `board/`: 게시판 관련 비즈니스 로직
    - `calendar/impl/`: 달력 기능 서비스
    - `magazinepdf/validate/`: 매거진 PDF 유효성 검증
    - `newcenter/`: 뉴센터 기능 서비스 (Link, Student 등 CRUD 구현)
    - `newcenterall/`: 뉴센터 통합 기능
    - `newsletter/`: 뉴스레터 기능
    - `tag/`: 태그 기능 (빈 폴더)
    - `validate/`: 공통 유효성 검증 (`AbstractValidateServiceImpl.java`)
...
### 3. 커스텀 설정 관리 (`webapp/_custom/unist/properties`)
...
### 4. 프론트엔드 공통 모듈 (`webapp/_custom/unist/_common/board/common`)
