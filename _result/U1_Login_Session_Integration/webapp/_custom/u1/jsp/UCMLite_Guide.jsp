...
<%@ page import="com.andwise.jw.auth.web.SessionMgr"%>
<%@ page import="com.andwise.jw.auth.vo.LoginVO"%>
...
<%
	...
	// 세션에서 로그인 정보 조회
	// ─────────────────────────────────────────────────────────────────
	LoginVO loginVO = SessionMgr.getLoginVO();
	
	// 세션 예외처리: 로그인 안된 경우 로그인 페이지로 이동
	if (loginVO == null || loginVO.getMemberId() == null) {
		response.sendRedirect("/cms/login/login.do");
		return;
	}
	...
	// [필수값] 사용자구분 - membertypeCd → userclass 변환
	// U1LoginController.java 매핑 기준:
	// Z(학부생, gbsinbun=S), V(대학원생, gbsinbun=G) → S(학생)
	// Y(직원, gbsinbun=J), X(조교, gbsinbun=D), W(교수, gbsinbun=P) → F(교직원)
	String userclass = "F";
	String membertypeCd = loginVO.getMembertypeCd();
	if ("Z".equals(membertypeCd) || "V".equals(membertypeCd)) {
		userclass = "S";
	}
